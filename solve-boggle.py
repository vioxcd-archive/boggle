...
def solve(board, keywords):
	...
	for keyword in keywords:
		first_letter_indexes = letters_index_lookup[keyword[0]]  # return all possible index
		
		# constant index search; search all possible index
		for index in first_letter_indexes:
			# search along periphery of index (a grid)
			letters_index = find_word_in_grid(keyword, index, board)

			if letters_index:  # find return None if not found; check here whether we found it
				result.append(letters_index)
